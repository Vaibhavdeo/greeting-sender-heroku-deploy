...
public class AppConstants {
	...
	public static final String UPDATE_QUERY = "IF EXISTS (SELECT * FROM GreetingDetails WHERE g_Id=? )
    UPDATE GreetingDetails SET (SP_DATE = ?, name = ?, subject=?, mail_Id=?) WHERE g_Id=?
ELSE
    INSERT INTO Table1 VALUES (g_Id, SP_DATE, subject ,name, mail_Id) values (?, ? , ? , ?, ?)";
